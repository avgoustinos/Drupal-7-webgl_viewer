...
function createRtiViewer(idDiv, imageUrl, width, height)
{
	...
	var canvasContainer = jQuery(jQuery("#"+idDiv)[0]);
	...
	var canvasNode = jQuery(jQuery("#"+idDiv + "_div")[0]);
	...
	divHelp.innerHTML = '<div id = "guideTable"><div id = "guideCell"> <div id = "guideList"><h3>WebRTIViewer<br/></h3><ul><li>Pan: LeftMouseButton + MouseMove.</li><li>Zoom in: MouseWhell or press the button <span><img src = "'+ baseUrl + 'sites/all/modules/webgl_viewer/css/icons/zoomin.png" alt=""/></span></li><li>Zoom out: MouseWheel or press the button <span><img src = "'+ baseUrl + 'sites/all/modules/webgl_viewer/css/icons/zoomout.png" alt=""/></span></li><li>To change the light direction: activate the light with the button <span><img src = "'+ baseUrl + 'sites/all/modules/webgl_viewer/css/icons/light.png" alt=""/></span> and press LeftMouseButton + MouseMove or Ctrl + LeftMouseButton + MouseMove.</li><li>Fullscreen: press the button <span><img src = "'+ baseUrl + 'sites/all/modules/webgl_viewer/css/icons/full.png" alt=""/></span> to active the fullscreen mode and the button <span><img src = "'+ baseUrl + 'sites/all/modules/webgl_viewer/css/icons/full_on.png" alt=""/></span> to exit</li><li>To reset the viewpoint: press R.</li></ul><a href="http://vcg.isti.cnr.it/rti/webviewer.php" target = "_black">Visual Computing Lab ISTI CNR Pisa</a></div></div></div>'
	divHelp.style.display = "none";
	canvasNode.append(divHelp);
	
	jQuery( "#"+idDiv + "_div #zoomIn" ).button({
      icons: {
	...
	jQuery( "#"+idDiv + "_div #zoomOut" ).button({
      icons: {
	...
	jQuery( "#"+idDiv + "_div #light" ).button({
      icons: {
	...
	jQuery( "#"+idDiv + "_div #help" ).button({
      icons: {
        primary: "helpIcon toolbarIcon"
      },
	  text: false,
      label: "Help"
    }).click(function(){jQuery( "#"+idDiv + "_guide" ).show();});
	...
	function fullScreenOn(){
		...
		var canvas = jQuery("#" + idDiv + "_webgl");
		...
		jQuery( "#"+idDiv + "_div #fullscreen").button( "option", options );
	};
	...
	jQuery( "#"+idDiv + "_div #fullscreen" ).button({
      icons: {
        primary: "fullIcon toolbarIcon"
      },
	  text: false,
	...
	function(){
			...
			if (!fullscreen)
				...
				fullScreenOff();
			jQuery("#"+idDiv + "_div #fullscreen").removeClass("ui-state-hover");
		}
	);
	...
	jQuery( "#"+idDiv + "_guide" ).click(function(){jQuery( "#"+idDiv + "_guide" ).hide();});
	...
	else
	{
		...
		function isRelitable(type){
			if (type ==  "IMAGE")
			{
				jQuery( "#"+idDiv + "_div #light" ).css("display", "none");
			}
...
function MultiResRenderer(gl, cacheSizeInBytes, onLoad)
 {
	...
	// box rendering
	/******************************************************/
	var bvs = sglLoadFile(baseUrl + "sites/all/modules/webgl_viewer/spidergl/box.v.glsl");
	var bfs = sglLoadFile(baseUrl + "sites/all/modules/webgl_viewer/spidergl/box.f.glsl");
...
MultiResRenderer.prototype = {
	...
	loadImage: function(url)
	{
		...
		{
			case 1:
				var vs = sglLoadFile(baseUrl + "sites/all/modules/webgl_viewer/spidergl/multi.v.glsl");
				var fs = sglLoadFile(baseUrl + "sites/all/modules/webgl_viewer/spidergl/hsh.f.glsl");
			...
			case 2:
				var vs = sglLoadFile(baseUrl + "sites/all/modules/webgl_viewer/spidergl/multi.v.glsl");
				var fs = sglLoadFile(baseUrl + "sites/all/modules/webgl_viewer/spidergl/lrgbptm.f.glsl");
				var prg = new SglProgram(this.gl, [vs], [fs]);
				log(prg.log);
				this._program  = prg;
				break;
			case 3:
				var vs = sglLoadFile(baseUrl + "sites/all/modules/webgl_viewer/spidergl/multi.v.glsl");
				var fs = sglLoadFile(baseUrl + "sites/all/modules/webgl_viewer/spidergl/rgbptm.f.glsl");
				var prg = new SglProgram(this.gl, [vs], [fs]);
				log(prg.log);
				this._program  = prg;
				break;
			case 4:
				var vs = sglLoadFile(baseUrl + "sites/all/modules/webgl_viewer/spidergl/multi.v.glsl");
				var fs = sglLoadFile(baseUrl + "sites/all/modules/webgl_viewer/spidergl/image.f.glsl");
				var prg = new SglProgram(this.gl, [vs], [fs]);
				log(prg.log);
				this._program  = prg;
				break;
